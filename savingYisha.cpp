...
void Metropolis(int, int, long&, double *, double& , double lamda);
// prints to file the results of the calculations
void output(int, int, int numprocs, double, int, double , int *);
...
int main(int argc, char* argv[])
{
  char *outfilename;
  long idum;
  int n_agents, n_trans,mcs, my_rank, numprocs,factor,factorMax,*counter,*total_counter;
  double *agent_matrix, average,total_average,M,dm,lamda;
  //  MPI initializations
  MPI_Init (&argc, &argv);
...
//initialize from command line
  ...
  */
  ...
  factorMax=ceil(n_agents*1.0/dm); //number of dm
  // Allocate memory for counter and initialize as 0
  counter=new int[factorMax+1]();
  total_counter=new int[factorMax+1]();
  ...
  for ( int cycles = myloop_begin; cycles <= myloop_end; cycles++){
      ...
      Metropolis(n_agents, n_trans,idum, agent_matrix,M,lamda);// transaction and calculate money in the system
      // update expectation values  for local node
      average+=M; //update money
      M=0.;
...
//      MPI_Reduce(&counter[i], &total_counter[i], 1, MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_WORLD);
      ...
      MPI_Reduce(counter, total_counter, size, MPI_INT, MPI_SUM, 0, MPI_COMM_WORLD);//note the MPI_type
    // print results
    if ( my_rank == 0) {
      output(n_agents, mcs,numprocs, total_average,factorMax,dm,total_counter);
...
void Metropolis(int n_agents, int n_trans, long& idum, double *agent_matrix,double& M,double lamda)
{
    double epsilon,deltaM;
    int i,j;
  // loop over random agent pair with 1e7 transactions
    for(int t=0;t<n_trans;t++){
        ...
        agent_matrix[j]=agent_matrix[j]-deltaM;

    }

    for(int i=0;i<n_agents;i++)  M+=agent_matrix[i];


} // end of Metropolis sampling over agent pair


void output(int n_agents, int mcs,int numprocs, double total_average,int factorMax,double dm,int* total_counter)
{
  ...
  for(int i=1;i<=factorMax;i++){
       ...
       ofile << setw(15) <<setprecision(8)<<(double)(total_counter[i]/numprocs)<<endl;
