...
void Metropolis(int, int, long&, double *, double& , double, int, double lamda, double*);
...
int main(int argc, char* argv[])
{
  ...
  double *agent_matrix,*counter,*total_counter, average,total_average,M,dm,lamda;
  //  MPI initializations
  MPI_Init (&argc, &argv);
...
//initialize from command line
  ...
  lamda=(double)atof(argv[5]);// [0.25 0.5 0.9] (if lamda=0, then no saving)
...
void Metropolis(int n_agents, int n_trans, long& idum, double *agent_matrix,double& M,double dm,int factorMax,double lamda, double* counter)
{
